...
def animate(i):
    global X, Y, dp, gnd_p, sigma, all_patches, patch, cropax, all_patchax, N, img
    ...
    if i < N:  # If the animation is still running
        ...
        for l in lamb:
            A = np.matmul(X,X.T)
            B = l*np.identity(A.shape[0])
            C = np.matmul(X, Y)
            g = np.matmul(np.linalg.inv(A+B), C)
            g = np.reshape(g, (29,45))
            ax[l].imshow(g)
            title = 'Lambda = ' + str(l)
            ax[l].set_title(title)
            fig.savefig('../temp_results/filters.png')
            response1 = ndimage.correlate(img, g)
            name1 = '../temp_results/correlate'+str(l)+'.png'
            mpimg.imsave(name1, response1, cmap='gray')
            response2 = ndimage.convolve(img, g)
            name2 = '../temp_results/convolve'+str(l)+'.png'
            mpimg.imsave(name2, response2, cmap='gray')
